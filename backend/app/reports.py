...
def report_html(analysis_id: str, db: Session = Depends(get_session)):
    ...
    try:
        ...
        keyword_block = summary.get("keyword") or {}
        semantic_block = summary.get("semantic") or {}
    ...
    keyword_score = normalize_score(keyword_block.get("score")) if keyword_block else None
    semantic_score = normalize_score(semantic_block.get("score")) if semantic_block else None
    overall_score = normalize_score(summary_block.get("overallScore")) if summary_block else None
    ...
    top_matches = []
    missing_concepts = []
    has_semantic = isinstance(semantic_block, dict) and bool(semantic_block)
    if has_semantic:
        top_matches = semantic_block.get("topMatches") or []
        missing_concepts = semantic_block.get("missingConcepts") or []

    top_matches_rows = "".join(
        f"<tr><td>{html_escape(m.get('resumeChunk'))}</td>"
        f"<td>{html_escape(m.get('jobChunk'))}</td>"
        f"<td>{html_escape(m.get('similarity'))}</td></tr>"
  ...
  <div class="container">
    ...
    <div class="card">
      ...
      <div class="grid" style="margin-top:12px;">
        <div class="kpi"><b>Keyword Score:</b>&nbsp;{(keyword_score if keyword_score is not None else score):.1f}%</div>
        {f'<div class="kpi"><b>Semantic Score:</b>&nbsp;{semantic_score:.1f}%</div>' if has_semantic and semantic_score is not None else ''}
      </div>
      <div class="muted" style="margin-top:6px;">
        Overall: {(overall_score if overall_score is not None else score):.1f}%
      </div>
      <div class="grid" style="margin-top:12px;">
    ...
    <div class="card">
      ...
      <table>
        <thead>
          <tr><th>Resume Chunk</th><th>Job Chunk</th><th>Similarity</th></tr>
        </thead>
        <tbody>
          {top_matches_rows}
        </tbody>
    ...
    <div class="card">
      <h2>Missing Concepts (Semantic)</h2>
      <ul style="margin-top:.5rem">{missing_concepts_html}</ul>
    </div>
