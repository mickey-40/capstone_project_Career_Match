...
TECH_HINTS = {
    ...
    "git","ci","cd","rest","graphql","oauth","jwt"
}
...
def extract_keywords(text: str) -> set[str]:
    ...
    return {t for t in normed if t and not _is_noise(t)}
...
class KeywordAnalyzer(BaseAnalyzer):
    """Keyword matcher with stopword filtering and tech-weighted scoring."""
    def run(self, resume_text: str, job_text: str) -> Dict:
        resume_set = extract_keywords(resume_text)
        job_set = extract_keywords(job_text)

        # Assign weights (tech terms get a boost)
        def weight(token: str) -> float:
            return 1.5 if token in TECH_HINTS else 1.0
